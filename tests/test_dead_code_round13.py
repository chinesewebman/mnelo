...
class TestRunStdioHappyPath:
    ...
    def test_run_stdio_with_mocked_stdio_server(self, monkeypatch):
        ...
        # [8/14 P1 fix] PEP 562 facade setattr doesn't work on modules. Patch the singleton
        # directly in sys.modules (shared across all imports in this process).
        guard_mod = _sys_mod.modules["mcp_guard"]
        original_mcp_avail = guard_mod._MCP_AVAILABLE
        try:
            guard_mod._MCP_AVAILABLE = True
            # stdio_server is value-binding `from mcp_guard import stdio_server` in
            # mcp_transports.run_stdio. mcp_transports line 54 also reads via facade
            # `sys.modules["mcp_server"].stdio_server`. Patch sys.modules['mcp_server'].
            _sys_mod.modules["mcp_server"].stdio_server = _MockStdio()

            # Mock server.run as async no-op (mcp_server.server is the MCP Server instance)
            async def fake_run(read, write, init):
                pass

            _mcp_repo.server.run = fake_run

            # Mock server.create_initialization_options as no-op
            def fake_init_opts():
                return {}

            _mcp_repo.server.create_initialization_options = fake_init_opts

            asyncio.run(_mcp_repo.run_stdio())
        finally:
            guard_mod._MCP_AVAILABLE = original_mcp_avail
...
class TestImportFallbackPath:
    ...
    def test_import_fallback_warning(self, monkeypatch, capsys):
        ...
        # [8/14 P1 fix] mcp_transports internal reads `_mg._MCP_AVAILABLE` (module
        # attribute lookup, value-binding bypass). Patch sys.modules['mcp_guard'] singleton.
        guard_mod = _sys_mod.modules["mcp_guard"]
        original_mcp_avail = guard_mod._MCP_AVAILABLE
        try:
