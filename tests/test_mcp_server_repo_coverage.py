...
class TestHandleSimpleAllBranches:
    ...
    def test_handle_simple_chunk_id_path(self, mem, clean_prefix):
        ...
        result = _mcp_repo._handle_simple(
            mem,
            "memory_remember",
            {
    ...
    def test_handle_simple_relation_id_path(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'rel_a_n', 'test_cov', ?, NULL)",
            (a_id, "2026-07-19T00:00:00"),
        )
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'rel_b_n', 'test_cov', ?, NULL)",
...
class TestGraphQueryBranch:
    ...
    def test_graph_query_handler(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'gq_target', 'test_cov', ?, NULL)",
