...
class TestHandleEntityResolveFullPath:
    ...
    def test_entity_resolve_with_kind_filter(self, mem):
        ...
        result = _mcp_repo._handle_entity_resolve(
            mem,
            {
                "kind": "stock",
                "threshold": 0.95,
...
class TestHandleListEntitiesFullPath:
    ...
    def test_list_entities_with_kind(self, mem, clean_prefix):
        ...
        for kind in ("test_kind_a", "test_kind_b"):
            mem._conn.execute(
                "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, ?, 'n', 'test_cov', ?, NULL)",
                (f"{clean_prefix}_{kind}", kind, "2026-07-19T00:00:00"),
            )
        mem._conn.commit()
        result = _mcp_repo._call_tool("memory_list_entities", {"kind": "test_kind_a"})
        data = json.loads(result)
    ...
    def test_list_entities_min_importance(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, importance, source, valid_from, valid_until) VALUES (?, 'test', 'low_imp', 0.1, 'test_cov', ?, NULL)",
            (f"{clean_prefix}_low", "2026-07-19T00:00:00"),
        )
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, importance, source, valid_from, valid_until) VALUES (?, 'test', 'high_imp', 0.9, 'test_cov', ?, NULL)",
            (f"{clean_prefix}_high", "2026-07-19T00:00:00"),
        )
        mem._conn.commit()
        result = _mcp_repo._call_tool("memory_list_entities", {"min_importance": 0.5})
        data = json.loads(result)
    ...
    def test_list_entities_limit(self, mem, clean_prefix):
        ...
        mem._conn.commit()
        result = _mcp_repo._call_tool("memory_list_entities", {"limit": 2})
        data = json.loads(result)
    ...
    def test_list_entities_excludes_deleted(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'active_n', 'test_cov', ?, NULL)",
            (f"{clean_prefix}_active", "2026-07-19T00:00:00"),
        )
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'deleted_n', 'test_cov', ?, ?)",
...
class TestHandleSearchRelationsFullPath:
    ...
    def test_search_relations_basic(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'sr_a_n', 'test_cov', ?, NULL)",
            (a_id, "2026-07-19T00:00:00"),
        )
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'sr_b_n', 'test_cov', ?, NULL)",
            (b_id, "2026-07-19T00:00:00"),
        )
        mem.relate(a_id, b_id, "r9_test_rel", weight=0.8)
        mem._conn.commit()
        result = _mcp_repo._call_tool("memory_search_relations", {"relation": "r9_test_rel"})
        data = json.loads(result)
        assert isinstance(data, dict)
    ...
    def test_search_relations_with_asof(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'sr2_a_n', 'test_cov', ?, NULL)",
            (a_id, "2026-07-19T00:00:00"),
        )
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'sr2_b_n', 'test_cov', ?, NULL)",
            (b_id, "2026-07-19T00:00:00"),
        )
    ...
    def test_search_relations_with_limit(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'sr3_n', 'test_cov', ?, NULL)",
            (a_id, "2026-07-19T00:00:00"),
        )
