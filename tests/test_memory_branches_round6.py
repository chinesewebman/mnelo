...
class TestForgetEntityBranch:
    ...
    def test_forget_entity_branch(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'forget_me', 'test_cov', ?, NULL)",
...
class TestEntityRecallAliasBoost:
    ...
    def test_alias_match_boosts_importance(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, aliases_json, importance, source, valid_from, valid_until) VALUES (?, 'test', 'boost_target', ?, 0.5, 'test_cov', ?, NULL)",
...
class TestGraphRecallSeedExpansion:
    ...
    def test_graph_recall_identity_fact_kind(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, summary, importance, source, valid_from, valid_until) VALUES (?, 'identity_fact', 'identity_name', 'identity summary', 0.9, 'test_cov', ?, NULL)",
            (eid, "2026-07-19T00:00:00"),
        )
        mem._conn.execute(
            "INSERT INTO chunks (id, content, source, timestamp, importance, valid_until) VALUES (?, ?, 'test_cov', ?, 0.5, NULL)",
...
class TestChineseBigramTokens:
    ...
    def test_chinese_query_returns_hits(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'concept', '测试中文实体名', 'test_cov', ?, NULL)",
            (eid, "2026-07-19T00:00:00"),
        )
        mem._conn.commit()
        hits = mem._entity_recall_with_conn(
    ...
    def test_ascii_single_char_token(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'concept', 'a thing', 'test_cov', ?, NULL)",
            (eid, "2026-07-19T00:00:00"),
        )
        mem._conn.commit()
        hits = mem._entity_recall_with_conn(
...
class TestEntityRecallSeenIdsDedup:
    ...
    def test_dedup_same_entity_in_two_kinds(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, summary, importance, source, valid_from, valid_until) VALUES (?, 'test', 'dedup_name', 'dedup summary', 0.7, 'test_cov', ?, NULL)",
            (eid, "2026-07-19T00:00:00"),
        )
        mem._conn.commit()
