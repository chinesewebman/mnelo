...
def mem():
    ...
    if sys.modules.get("memory", None) is None or not getattr(sys.modules["memory"], "__file__", "").startswith(str(_REPO)):
        # Force REPO
        from pathlib import Path
        import importlib.util as _ilu
...
class TestGetAliasesEmptyEverything:
    ...
    def test_empty_name_no_aliases_returns_empty(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, aliases_json, source, valid_from, valid_until) VALUES (?, 'test', '', '[]', 'test_cov', ?, NULL)",
    ...
    def test_empty_name_none_aliases_returns_empty(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, aliases_json, source, valid_from, valid_until) VALUES (?, 'test', '', NULL, 'test_cov', ?, NULL)",
            (eid, "2026-07-19T00:00:00"),
        )
        mem._conn.commit()
        with _memory_repo._with_row_factory(mem._conn, sqlite3.Row):
    ...
    def test_get_aliases_deleted_entity_returns_empty(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, aliases_json, source, valid_from, valid_until) VALUES (?, 'test', 'del_n', '[\"x\"]', 'test_cov', ?, ?)",
    ...
    def test_get_aliases_with_name_returns_name(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, aliases_json, source, valid_from, valid_until) VALUES (?, 'test', 'real_name_xyz', '[]', 'test_cov', ?, NULL)",
            (eid, "2026-07-19T00:00:00"),
        )
        mem._conn.commit()
        with _memory_repo._with_row_factory(mem._conn, sqlite3.Row):
...
class TestFindDuplicateSameIdSkipped:
    ...
    def test_same_id_entities_skipped(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, ?, 'same_name', 'test_cov', ?, NULL)",
...
class TestMergeSameIdFails:
    ...
    def test_merge_same_id_returns_false(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'same_n', 'test_cov', ?, NULL)",
            (eid, "2026-07-19T00:00:00"),
        )
        mem._conn.commit()
        ...
        row = mem._conn.execute("SELECT valid_until FROM entities WHERE id = ?", (eid,)).fetchone()
...
class TestMergeMissingEntityFails:
    ...
    def test_merge_missing_primary_returns_false(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'b_n', 'test_cov', ?, NULL)",
            (b_id, "2026-07-19T00:00:00"),
        )
        mem._conn.commit()
        result = entity_resolve.merge_entities(
            mem._conn,
            f"{clean_prefix}_missing_a",
            b_id,
        )
        assert result is False
    ...
    def test_merge_missing_secondary_returns_false(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'a_n', 'test_cov', ?, NULL)",
...
class TestMergeAlreadyDeletedFails:
    ...
    def test_merge_deleted_primary_returns_false(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'a_n', 'test_cov', ?, '2026-07-19T01:00:00')",
            (a_id, "2026-07-19T00:00:00"),
        )
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'b_n', 'test_cov', ?, NULL)",
...
class TestGetAliasesAliasesJsonVariants:
    ...
    def test_aliases_dict_format(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, aliases_json, source, valid_from, valid_until) VALUES (?, 'test', 'good_n', ?, 'test_cov', ?, NULL)",
...
class TestMergeSuccessfulCases:
    ...
    def test_merge_with_empty_aliases(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, aliases_json, source, valid_from, valid_until) VALUES (?, 'test', 'a_n', '[]', 'test_cov', ?, NULL)",
            (a_id, "2026-07-19T00:00:00"),
        )
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, aliases_json, source, valid_from, valid_until) VALUES (?, 'test', 'b_n', '[]', 'test_cov', ?, NULL)",
            (b_id, "2026-07-19T00:00:00"),
        )
        mem._conn.commit()
        result = entity_resolve.merge_entities(mem._conn, a_id, b_id)
