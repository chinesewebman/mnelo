...
class TestMemoryCRUD(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_id_prefix = "test_crud_" + datetime.now().strftime("%H%M%S_%f") + f"_{cls.__name__}"
        print(f"\n--- {cls.test_id_prefix} ---")
...
class TestEntityResolve(unittest.TestCase):
    ...
    def test_01_merge_candidates(self):
        ...
        # [7/20 v0.5.9] Pass ids=[...] to scope scan to test entities only.
        # Without this, the function exits at max_pairs=500 cap when LIVE DB
        # has 50+ stock entities (~1225 pairs needed to reach ours).
        cands = find_duplicate_candidates(self.mem._conn, threshold=0.7, ids=[a_id, b_id])
    ...
    def test_03_merge_uses_get_aliases_helper(self):
        ...
        b_row = self.mem._conn.execute("SELECT aliases_json FROM entities WHERE id = ?", (b_id,)).fetchone()
...
class TestP0BoundsCheck(unittest.TestCase):
    ...
    def test_08_remember_importance_clamped(self):
        ...
        row = self.mem._conn.execute("SELECT importance FROM chunks WHERE id = ?", (cid,)).fetchone()
    ...
    def test_10_relate_weight_clamped(self):
        ...
        row = self.mem._conn.execute("SELECT weight FROM relations WHERE id = ?", (rid,)).fetchone()
        self.assertEqual(row[0], 1.0)
        # cleanup
    ...
    def test_11_update_new_importance_clamped(self):
        ...
        row = self.mem._conn.execute("SELECT importance FROM chunks WHERE id = ?", (new_id,)).fetchone()
        self.assertEqual(row[0], 1.0)
        # cleanup
        self.mem.forget(cid)
    ...
    def test_12_upsert_entity_importance_clamped(self):
        ...
        row = self.mem._conn.execute("SELECT importance FROM entities WHERE id = ?", (ent_id,)).fetchone()
