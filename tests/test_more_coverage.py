...
class TestEntityResolve:
    ...
    def test_find_duplicate_candidates_with_match(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'stock', 'sh600089', 'test_cov', ?, NULL)",
...
class TestMCPServerHandlers:
    ...
    def test_handle_search_relations(self, mem, clean_prefix):
        ...
        # 建一个 relation
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'a', 'test_cov', ?, NULL)",
            (f"{clean_prefix}_src", "2026-07-19T00:00:00"),
        )
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'b', 'test_cov', ?, NULL)",
    ...
    def test_handle_entity_resolve(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, ?, 'sh600089', 'test_cov', ?, NULL)",
...
class TestMCPServerRateLimit:
    ...
    def test_rate_limit_exceeded_raises(self):
        ...
        # [8/14 fix] `from mcp_server import config` 拿到的是 config module (not the
        # singleton instance). config.config 是 instance, .rate_limit_max_per_window 才是 attr.
        from config import config as _cfg_instance

        RATE_LIMIT_MAX_REQS = _cfg_instance.rate_limit_max_per_window
...
class TestCallToolErrorPaths:
    ...
    def test_call_tool_rate_limited(self, monkeypatch):
        ...
        # [8/14 fix] facade mcp_server is a plain module (no ModuleType subclass),
        # 所以 PEP 562 __setattr__ 在外部赋值时不被 invoke. 直接 patch sub-module.
        import mcp_tool_dispatcher

        monkeypatch.setattr(mcp_tool_dispatcher, "_rate_limit_check", _raise_rate)
...
class TestRunSSEEntryPoints:
    ...
    def test_run_sse_rejects_mcp_unavailable(self, monkeypatch):
        ...
        # [8/14 P1 fix v2] mcp_transports 内部改用 module attribute lookup (c9697f8 fix):
        #   `_mg._MCP_AVAILABLE` 读 mcp_guard._MCP_AVAILABLE, 不是 mcp_transports 自己的.
        # 所以 patch 直 mcp_guard module (单 instance across sys.modules).
        import mcp_guard as _mg
        from mcp_transports import run_sse

        monkeypatch.setattr(_mg, "_MCP_AVAILABLE", False)
    ...
    def test_run_sse_port_in_use_exits_cleanly(self, monkeypatch):
        ...
        monkeypatch.setattr(mcp_transports, "_check_port_available", lambda h, p: False)
        # 不抛, 优雅 return
    ...
    def test_run_sse_token_load_fail_raises(self, monkeypatch):
        ...
        # [8/14 fix] facade 是 plain module, PEP 562 __setattr__ 不被外部赋值 invoke.
        # 直接 patch auth.load_auth_token. run_sse 在 mcp_transports 内用 `from auth import
        # load_auth_token` (value-binding), 所以 patch auth module 生效.
        ...
        monkeypatch.setattr(auth, "load_auth_token", _fail)
        from mcp_transports import run_sse
...
class TestMergeEntities:
    ...
    def test_merge_two_entities(self, mem, clean_prefix):
        ...
        for eid in (a, b):
            mem._conn.execute(
                "INSERT INTO entities (id, kind, name, aliases_json, source, valid_from, valid_until) VALUES (?, 'stock', ?, '[]', 'test_cov', ?, NULL)",
        ...
        row_b = mem._conn.execute("SELECT valid_until FROM entities WHERE id = ?", (b,)).fetchone()
