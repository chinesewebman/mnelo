...
class TestRelateEdgeCases(unittest.TestCase):
    ...
    def test_02_relate_with_extra_properties(self):
        """properties JSON 字段."""
        rid = self.mem.relate(self.a_id, self.b_id, "tags_于", weight=0.5, properties={"tag": "test", "quantity": 100})
        row = self.mem._conn.execute("SELECT properties_json FROM relations WHERE id = ?", (rid,)).fetchone()
    ...
    def tearDownClass(cls):
        with cls.mem._conn:
            ...
            cls.mem._conn.execute("DELETE FROM relations WHERE source_id LIKE ? OR target_id LIKE ?", (f"{cls.test_id_prefix}%", f"{cls.test_id_prefix}%"))
...
class TestRecallEdgeCases(unittest.TestCase):
    ...
    def test_07_recall_filters_specific(self):
        ...
        results = self.mem.recall("Trinity 三层报告", top_k=3, strategy="vector_only", filters={"source": "trinity_daily:part1"})
...
class TestUpdateEdgeCases(unittest.TestCase):
    ...
    def test_02_double_update_same_id(self):
        ...
        cid = self.mem.remember(f"{self.test_id_prefix}: 双重 update", source="test-update", importance=0.5)
        ...
        r1 = self.mem._conn.execute("SELECT superseded_by FROM chunks WHERE id = ?", (cid,)).fetchone()
        r2 = self.mem._conn.execute("SELECT superseded_by FROM chunks WHERE id = ?", (new_id_1,)).fetchone()
...
class TestForgetEdgeCases(unittest.TestCase):
    ...
    def test_01_forget_cascade_false(self):
        ...
        e_after = self.mem._conn.execute("SELECT valid_until FROM entities WHERE id = ?", (eid,)).fetchone()
        ...
        r_after = self.mem._conn.execute("SELECT valid_until FROM relations WHERE id = ?", (rid,)).fetchone()
        print(f"  ✅ forget(cascade=False) → entity valid_until={e_after['valid_until']}, relation valid_until={r_after['valid_until'] if r_after['valid_until'] else '(not cascaded, 触发器自动)'}")
...
class TestStatsIntegrity(unittest.TestCase):
    ...
    def test_01_stats_fields(self):
        ...
        for k in ["entities", "chunks", "relations"]:
            self.assertEqual(s[k]["total"], s[k]["active"] + s[k]["deleted"], f"stats[{k}] total != active + deleted")

        print(f"  ✅ stats 完整 → entities {s['entities']['total']}, chunks {s['chunks']['total']}, relations {s['relations']['total']}, vectors {s['vectors']}, recall_log {s['recall_log']}")
...
class TestEmbedder(unittest.TestCase):
    ...
    def test_01_embedder_loaded(self):
        ...
        self.assertIn(_embed_mod.EMBED_DIM, (256, 384, 512, 768, 1024), f"dim={_embed_mod.EMBED_DIM} 不在 fastembed 常见输出维度集合里")
...
class TestVec0RowFactory(unittest.TestCase):
    ...
    def test_03_mcp_recall_full_path(self):
        ...
        [8/14 P1 fix] rewrite to use in-process MCP server fixture instead of relying on
        port 8086 + external hermes MCP server instance. The previous version assumed a
        live hermes server was bound to 8086 — flaky on dev boxes where multiple instances
        collide.
        """
        import asyncio
        import json
        ...
        # [8/14 P1 fix] bring up our own MCP SSE server on an ephemeral port, then test
        # against it. Original test assumed external server; we make it self-contained.
        import uvicorn
        import socket as _sock_mod

        # Pick a free port (kernel assigned via bind(0))
        probe = _sock_mod.socket(_sock_mod.AF_INET, _sock_mod.SOCK_STREAM)
        probe.bind(("127.0.0.1", 0))
        port = probe.getsockname()[1]
        probe.close()

        # Build the same app the real server uses
        from mcp_server import _build_sse_app
        from auth import load_auth_token
        from pathlib import Path as _Path

        _token_path = _Path.home() / ".config" / "mnelo" / "auth_token"
        if not _token_path.exists():
            self.skipTest("mnelo auth_token file not present; skipping MCP e2e test")
        _live_token = _token_path.read_text().strip()

        app = _build_sse_app(_live_token)
        ...
        thread.start()
        # Wait for server to be ready (poll up to 3s)
        for _ in range(30):
            time.sleep(0.1)
            if server.started:
                break
        else:
            self.skipTest(f"ephemeral SSE test server failed to start on port {port}")
