...
class TestForgetBranches:
    ...
    def test_forget_entity(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'test_forget', 'test_cov', ?, NULL)",
    ...
    def test_forget_relation(self, mem, clean_prefix):
        ...
        row = mem._conn.execute("SELECT source_id, target_id FROM relations WHERE id = ?", (rid_int,)).fetchone()
...
class TestIdentityFactImmutability:
    ...
    def test_identity_fact_update_blocked(self, mem, clean_prefix):
        ...
        row = mem._conn.execute("SELECT name FROM entities WHERE id = ? AND valid_until IS NULL", (eid,)).fetchone()
...
class TestGraphQuery:
    ...
    def test_bfs_hops(self, mem, clean_prefix):
        ...
        mem._conn.execute("INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'a', 'test_cov', ?, NULL)", (a, "2026-07-19T00:00:00"))
        mem._conn.execute("INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'b', 'test_cov', ?, NULL)", (b, "2026-07-19T00:00:00"))
        mem._conn.execute("INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'c', 'test_cov', ?, NULL)", (c, "2026-07-19T00:00:00"))
    ...
    def test_empty_start_node_no_edges(self, mem, clean_prefix):
        ...
        mem._conn.execute("INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'isolated', 'test_cov', ?, NULL)", (a, "2026-07-19T00:00:00"))
        mem._conn.commit()
        result = mem.graph_query(a, max_hops=2)
...
class TestValidationEdges:
    ...
    def test_entity_importance_nan_rejected(self):
        with pytest.raises(ValidationError, match="NaN"):
            validate_entity_payload({"id": "x", "kind": "test", "importance": float("nan")})

    def test_entity_importance_clamped(self):
        out = validate_entity_payload({"id": "x", "kind": "test", "importance": 5.0})
        assert out["importance"] == 1.0
        out = validate_entity_payload({"id": "x", "kind": "test", "importance": -0.5})
...
class TestAuthTokenFile:
    ...
    def test_load_raises_on_missing_token(self, monkeypatch):
        ...
        monkeypatch.delenv("MNELO_AUTH_TOKEN", raising=False)
        import auth as auth_mod
        import os
...
class TestExtraCoverageGaps:
    ...
    def test_graph_query_with_edge_types_filter(self, mem, clean_prefix):
        ...
        mem._conn.execute("INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'ga', 'test_cov', ?, NULL)", (a, "2026-07-19T00:00:00"))
        mem._conn.execute("INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'gb', 'test_cov', ?, NULL)", (b, "2026-07-19T00:00:00"))
