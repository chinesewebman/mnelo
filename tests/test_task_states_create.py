...
def _setup(m):
    ...
    try:
        m._conn.execute(
            "DELETE FROM task_states WHERE task_id LIKE 'task:tlm5-%' "
            "OR task_id LIKE 'loop:tlm5-%'"
        )
        m._conn.execute(
            "DELETE FROM entities WHERE id LIKE 'task:tlm5-%' "
...
def test_task_create_basic_no_loop():
    ...
    try:
        ...
        # 校验 DB
        ent = tuple(m._conn.execute(
            "SELECT kind, name, memory_type FROM entities WHERE id=?",
            (r["task_id"],),
        ).fetchone())
        assert ent == ("task", "独立一次性任务", "ephemeral")

        win = tuple(m._conn.execute(
            "SELECT state, valid_from, valid_until, reason FROM task_states "
...
def test_task_create_with_loop_sets_active():
    ...
    try:
        ...
        loop_r = ts_mod.loop_create(
            m._conn, name="耗材库存", trigger="库存低于阈值",
...
def test_terminal_bookkeeping_clears_active_task():
    ...
    try:
        _setup(m)
        loop_r = ts_mod.loop_create(
            m._conn, name="耗材", trigger="库存低",
            interval_hours=24, now="2026-08-06T09:00",
        )
        lid = loop_r["loop_id"]

        task_r = ts_mod.task_create(
            m._conn, name="采购", loop_id=lid,
            now="2026-08-06T10:00",
        )
        tid = task_r["task_id"]
