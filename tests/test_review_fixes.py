...
def test_rf1_replay_includes_intermediate_state():
    ...
    try:
        ...
        for state, vf, vu in windows:
            m._conn.execute(
                "INSERT INTO task_states (task_id, state, valid_from, valid_until, created_at) VALUES (?, ?, ?, ?, ?)",
                (tid, state, vf, vu, vf),
            )
        m._conn.commit()
...
def test_rf2_chinese_name_uses_pinyin_slug():
    ...
    try:
        ...
        assert "task" not in r["task_id"].rsplit("-", 1)[-1].split(":"), f"slug 退化: {r['task_id']}"
...
def test_rf6_transition_docstring_mentions_transaction():
    ...
    assert "transaction" in doc.lower() or "事务" in doc, "transition() docstring 需明示事务 / 原子性要求"
...
def test_rf7_list_typing_imported():
    ...
    assert hasattr(task_states, "List") or hasattr(__import__("typing"), "List"), "typing.List 未导入"
    # 实际尝试注解 List[...]
    try:
        ...
        assert "from typing import List" in text or ", List" in text, "typing.List 导入未在 task_states.py"
    except ImportError:
...
def test_rf17_no_negative_window_when_now_rewound():
    ...
    try:
        ...
        ts_mod.transition(
            m._conn,
            task_id="task:rf17-rewind",
            to_state="in_progress",
            reason="rewind",
            now="2026-08-06T10:00:05",
        )
        ...
        wins = [
            tuple(r)
            for r in m._conn.execute(
                "SELECT state, valid_from, valid_until FROM task_states WHERE task_id='task:rf17-rewind' ORDER BY id",
            )
        ]
...
def test_rf11_now_override_seconds_strict_increasing():
    ...
    try:
        ...
        ts_mod.transition(
            m._conn,
            task_id="task:rf11-sec",
            to_state="in_progress",
            reason="A",
            now="2026-08-06T10:00:05",  # 秒级
        )
        ts_mod.transition(
            m._conn,
            task_id="task:rf11-sec",
        ...
        wins = [
            tuple(r)
            for r in m._conn.execute(
                "SELECT state, valid_from, valid_until FROM task_states WHERE task_id='task:rf11-sec' ORDER BY id",
            )
        ]
...
def test_rf13_loop_update_soft_deleted_loop_rejected():
    ...
    try:
        ...
        m._conn.execute(
            "INSERT INTO entities (id, kind, name, properties_json, valid_from, valid_until) VALUES (?, ?, ?, ?, ?, ?)",
