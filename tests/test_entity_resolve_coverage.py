...
class TestGetAliases:
    ...
    def test_entity_with_empty_name_and_no_aliases(self, mem, clean_prefix):
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', '', 'test_cov', ?, NULL)",
    ...
    def test_entity_with_bad_aliases_json(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, aliases_json, source, valid_from, valid_until) VALUES (?, 'test', 'good_name', 'not_valid_json{{', 'test_cov', ?, NULL)",
    ...
    def test_entity_with_aliases_json_list(self, mem, clean_prefix):
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, aliases_json, source, valid_from, valid_until) VALUES (?, 'test', 'primary', ?, 'test_cov', ?, NULL)",
            (f"{clean_prefix}_with_aliases", json.dumps(["alt1", "alt2"]), "2026-07-19T00:00:00"),
        )
        mem._conn.commit()
...
class TestFindDuplicateCandidates:
    ...
    def test_kind_with_single_entity_skipped(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, ?, 'only_one', 'test_cov', ?, NULL)",
            (f"{clean_prefix}_solo", f"test_kind_{clean_prefix[-6:]}", "2026-07-19T00:00:00"),
        )
        mem._conn.commit()
        result = entity_resolve.find_duplicate_candidates(
            mem._conn,
            threshold=0.5,
            kind=f"test_kind_{clean_prefix[-6:]}",
            max_pairs=10,
        )
        assert isinstance(result, list)
    ...
    def test_empty_name_entities_skipped(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, ?, '', 'test_cov', ?, NULL)",
            (f"{clean_prefix}_empty_a", unique_kind, "2026-07-19T00:00:00"),
        )
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, ?, 'has_name', 'test_cov', ?, NULL)",
    ...
    def test_alias_conflict_match_path(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, aliases_json, source, valid_from, valid_until) VALUES (?, ?, 'apple_inc', ?, 'test_cov', ?, NULL)",
            (f"{clean_prefix}_alias_a", unique_kind, json.dumps(["AAPL", "apple_corp"]), "2026-07-19T00:00:00"),
        )
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, aliases_json, source, valid_from, valid_until) VALUES (?, ?, 'apple_corporation', ?, 'test_cov', ?, NULL)",
            (f"{clean_prefix}_alias_b", unique_kind, json.dumps(["AAPL", "aapl_company"]), "2026-07-19T00:00:00"),
        )
        mem._conn.commit()
        result = entity_resolve.find_duplicate_candidates(
            mem._conn,
        ...
        has_alias_conflict = any(r[2] == 1.0 and "alias" in r[3].lower() for r in result)
    ...
    def test_high_threshold_returns_empty(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, ?, 'foo', 'test_cov', ?, NULL)",
            (f"{clean_prefix}_hi_a", unique_kind, "2026-07-19T00:00:00"),
        )
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, ?, 'bar', 'test_cov', ?, NULL)",
...
class TestMergeEntities:
    ...
    def test_same_id_returns_false(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'name', 'test_cov', ?, NULL)",
    ...
    def test_missing_primary_returns_false(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'b_name', 'test_cov', ?, NULL)",
    ...
    def test_missing_secondary_returns_false(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, 'test', 'a_name', 'test_cov', ?, NULL)",
    ...
    def test_merge_two_entities_success(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, aliases_json, source, valid_from, valid_until) VALUES (?, 'test', 'primary_name', ?, 'test_cov', ?, NULL)",
            (a_id, json.dumps(["alias1"]), "2026-07-19T00:00:00"),
        )
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, aliases_json, source, valid_from, valid_until) VALUES (?, 'test', 'secondary_name', ?, 'test_cov', ?, NULL)",
...
class TestFindDuplicatesReport:
    ...
    def test_with_candidates_returns_table(self, mem, clean_prefix):
        ...
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, source, valid_from, valid_until) VALUES (?, ?, 'apple', 'test_cov', ?, NULL)",
