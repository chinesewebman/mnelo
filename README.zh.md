...
### 🌏 开箱即用双语
- locale 自动检测：`MNELO_MEMORY_LANG` > `LC_ALL` > `LANG` > `en`
- 30+ 用户可见字符串，全部双语
...
│   ├── migrate_to_mnelo.py ← 从旧 Mnemosyne → mnelo
...
设 `MNELO_MEMORY_LANG=ja` 测试。locale 缺失 fallback 到 `en`，再到 `msg_id`（缺失字符串可调试，不是沉默失败）。
