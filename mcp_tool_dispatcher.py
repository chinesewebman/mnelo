...
if _MCP_AVAILABLE:
    ...
    # _g(data, key, default) 替代 30+ 处 `data.get(K, default) if isinstance(data, dict) else default`.
    def _g(data, key, default=None):
        """Safe-get for echo builders — data may be dict / list / scalar."""
        if isinstance(data, dict):
            return data.get(key, default)
        return default

    def _echo_remember(args, data):
        cid = _g(data, "chunk_id", "?")
        imp = args.get("importance", "?")
        return f"{_ECHO} {_ECHO_LABEL}    +{cid}  (importance={imp})"

    def _echo_recall(args, data):
        hits = data if isinstance(data, list) else []
        query = args.get("query", "")[:30]
        if hits:
            top = hits[0] if isinstance(hits[0], dict) else {}
            method = _g(top, "method", "?")
            rrf = _g(top, "rrf_score", "?")
            try:
                rrf = f"{float(rrf):.4f}"
    ...
    def _echo_forget(args, data):
        target = args.get("target_id", "?")
        kind = args.get("target_kind", "chunk")
        purged = _g(data, "queued_purge", 1)
        return f"{_ECHO} {_ECHO_LABEL}    -{kind}:{target}  ({purged} queued)"

    def _echo_update(args, data):
        new_cid = _g(data, "new_chunk_id", "?")
        old = args.get("old_id", "?")
        return f"{_ECHO} {_ECHO_LABEL}    ↻{new_cid}  (supersedes {old})"

    def _echo_relate(args, data):
        src = args.get("source_id", "?")
        tgt = args.get("target_id", "?")
        rel = args.get("relation", "?")
        return f"{_ECHO} {_ECHO_LABEL}    ⟶{src}→{tgt}  ({rel})"

    def _echo_graph_query(args, data):
        nodes = _g(data, "nodes", []) or []
        edges = _g(data, "edges", []) or []
        start = args.get("start_node", "?")
        return f"{_ECHO} {_ECHO_LABEL}    ⌘{start}  ({len(nodes)} nodes, {len(edges)} edges)"

    def _echo_stats(args, data):
        chunks_obj = _g(data, "chunks") or {}
        ents_obj = _g(data, "entities") or {}
        chunks = chunks_obj.get("active", "?") if isinstance(chunks_obj, dict) else "?"
        ents = ents_obj.get("active", "?") if isinstance(ents_obj, dict) else "?"
        vecs = _g(data, "vectors", "?")
        return f"{_ECHO} {_ECHO_LABEL}    stats: chunks={chunks} entities={ents} vectors={vecs}"

    def _echo_entity_resolve(args, data):
        cands = _g(data, "candidates", []) or []
        thresh = args.get("threshold", 0.85)
        return f"{_ECHO} {_ECHO_LABEL}    ≡{len(cands)} dup candidates  (threshold={thresh})"

    def _echo_list_entities(args, data):
        if isinstance(data, list):
            ents = data
        else:
            ents = _g(data, "entities", []) or []
        kind = args.get("kind", "all")
        return f"{_ECHO} {_ECHO_LABEL}    ⊃{len(ents)} entities  (kind={kind})"

    def _echo_search_relations(args, data):
        rels = _g(data, "relations", []) or []
        rel_type = args.get("relation", "all")
        return f"{_ECHO} {_ECHO_LABEL}    ⇢{len(rels)} relations  (type={rel_type})"

    def _echo_task_create(args, data):
        task_id = _g(data, "task_id", "?")
        state = _g(data, "current_state", "?")
        return f"{_ECHO} {_ECHO_LABEL}    +{task_id}  (state={state})"

    def _echo_task_transition(args, data):
        task_id = _g(data, "task_id", "?")
        from_state = _g(data, "from_state", "?")
        to_state = _g(data, "to_state", "?")
        wid = _g(data, "window_id", "?")
        return f"{_ECHO} {_ECHO_LABEL}    {task_id}  {from_state}→{to_state}  (win={wid})"

    def _echo_task_list(args, data):
        tasks = _g(data, "tasks", []) or []
        truncated = _g(data, "truncated", False)
        return f"{_ECHO} {_ECHO_LABEL}    ⊃{len(tasks)} tasks  (truncated={truncated})"

    def _echo_task_replay(args, data):
        wc = _g(data, "window_count", "?")
        cs = _g(data, "current_state", "?")
        tid = _g(data, "task_id", "?")
        return f"{_ECHO} {_ECHO_LABEL}    ↻{tid}  ({wc} windows, current={cs})"

    def _echo_loop_create(args, data):
        lid = _g(data, "loop_id", "?")
        ih = _g(data, "interval_hours", "?")
        return f"{_ECHO} {_ECHO_LABEL}    ⊕{lid}  (interval={ih}h)"

    def _echo_loop_tick(args, data):
        verdict = _g(data, "verdict", "?")
        lid = _g(data, "loop_id", "?")
        return f"{_ECHO} {_ECHO_LABEL}    ⏱{lid}  verdict={verdict}"

    def _echo_loop_update(args, data):
        lid = _g(data, "loop_id", "?")
        changed = _g(data, "changed", {}) or {}
        return f"{_ECHO} {_ECHO_LABEL}    ✎{lid}  changed={list(changed.keys() if hasattr(changed, 'keys') else [])}"

    def _echo_loop_list(args, data):
        loops = _g(data, "loops", []) or []
        return f"{_ECHO} {_ECHO_LABEL}    ⊃{len(loops)} loops"

    # Dispatch table: tool name → echo builder
    _ECHO_BUILDERS = {
        "memory_remember": _echo_remember,
        "memory_recall": _echo_recall,
        "memory_forget": _echo_forget,
        "memory_update": _echo_update,
        "memory_relate": _echo_relate,
        "memory_graph_query": _echo_graph_query,
        "memory_stats": _echo_stats,
        "memory_entity_resolve": _echo_entity_resolve,
        "memory_list_entities": _echo_list_entities,
        "memory_search_relations": _echo_search_relations,
        "memory_task_create": _echo_task_create,
        "memory_task_transition": _echo_task_transition,
        "memory_task_list": _echo_task_list,
        "memory_task_replay": _echo_task_replay,
        "memory_loop_create": _echo_loop_create,
        "memory_loop_tick": _echo_loop_tick,
        "memory_loop_update": _echo_loop_update,
        "memory_loop_list": _echo_loop_list,
    }
