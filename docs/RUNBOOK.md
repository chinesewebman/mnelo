...
实战部署约 5 分钟。**单 SQLite 文件、macOS launchd 守护, MCP 协议 via SSE**.
...
hits = client.recall("实战建仓 5symbols", top_k=5, strategy='rrf')
for h in hits:
...
vector   chunk_20260718_072606_654597   实战 sh600089 建仓 12,000 @ 18.96
...
| `memory_recall` | 4 路召回 + RRF (实战 p95=36ms) |
| `memory_relate` | 加实体关系 |
...
# 给 LLM 用作 context (实战 top-3 RRF 总是最有意义的)
...
⚠️  concept 占 91.0% — 实战 kind 单一化, 考虑提升其他 kind 占比
```
...
`memory.db` 是单 SQLite 文件, 实战 cp 即可 (WAL 用 `-wal` flag):
...
python3 scripts/repair_vectors.py --apply   # 实战写入 (1591 条历史, ~2 sec)
```
...
⚠️ 当前版本**单用户本地优先**. 多租户隔离还没建. 实战风险:

1. **MCP server 监听 127.0.0.1** — 不暴露对外.
2. **schema 用 valid_until 软删除** — 实战安全敏感数据记得 **手动 `forget()`**.
