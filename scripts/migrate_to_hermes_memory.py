...
def import_from_export(export_file: Path, batch_size: int = 100):
    ...
    for t in triples:
        ...
        for ent_id, label in [(t['subject'], 'subject'), (t['object'], 'object')]:
            ...
            if not existing:
                ...
                """, (ent_id, ent_id, json.dumps({'triple_role': label}, ensure_ascii=False), t.get('valid_from') or now()))
    ...
    for cf in canonical_facts:
        ...
        """, (
            ent_id, cf['name'], (cf.get('body') or '')[:200],
            json.dumps({
                'canonical_id': cf['id'],
    ...
    for a in annotations:
        ...
        """, (
            target_chunk, target_entity, f"has_{a['kind']}",
