...
[7/18 patch F] 实战 i18n 框架 — 支持中英双语 (more locales easy to add)
- 实战 locale 检测: env LANG / LC_ALL > config.toml > 'en' (default)
- 实战 fallback 链: zh → en (实战 zh fallback 到 en)
- 实战 t() 函数: msg_id → 实战 string. 支持 format args.

设计:
1. 实战 message 不在代码里写死 string, 用 msg_id (e.g. 'db.stats.retrieved')
2. 实战 table 在 i18n_messages.py: MESSAGES = {'msg_id': {'zh': '...', 'en': '...'}, ...}
...
def _normalize(lang: str) -> str:
    """实战: zh_CN.UTF-8 → zh, en-US → en, zh-TW → zh-TW (实战)."""
    if not lang:
        return 'en'
    ...
    # 简化: zh_CN/zh_TW 都归 'zh' (实战主用户实战 zh)
    if primary == 'zh':
        return 'zh'
    return primary
...
def current_locale() -> str:
    """实战: 返回缓存的当前 locale (实战 use 之后 reload())."""
...
def reload() -> None:
    """实战: 重新检测 locale (实战 HERMES_MEMORY_LANG 实测 changed 后 reload)."""
    global _current_locale, _current_locale_cache
    _current_locale = get_locale()
    _current_locale_cache = _current_locale
...
def t(msg_id: str, **kwargs) -> str:
    """实战: 实战 message resolver.

    Args:
        msg_id: 实战唯一 id (e.g. 'db.stats.retrieved')
        **kwargs: format args (e.g. count=4390)

    Returns:
        实战当前 locale 的实战 string. 实战 fallback: msg_id 未找到 → 'en' 表; 都没有 → msg_id 本身.
    """
    from i18n_messages import MESSAGES
    loc = current_locale()
    table = MESSAGES.get(msg_id, {})
    # 实战当前 locale, fallback 到 'en'
